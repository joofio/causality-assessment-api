...
function(req,res){
  ...
  if(length(pharmagroup_r) >0) { vec <- c(vec, as.character("PharmaGroup")) 
  st <- c(st, as.character(data$PHARMAGROUP)) } 
  if(length(notifier_r) >0) { vec <- c(vec, as.character("Notifier")) 
  st <- c(st, as.character(data$NOTIFIER)) } 
  if(length(concomitant_r) >0) { vec <- c(vec, as.character("Concomitant")) 
  st <- c(st, as.character(data$SUSPENDED)) } 
  if(length(interact_r) >0) { vec <- c(vec, as.character("SuspectedInteraction")) 
  st <- c(st, as.character(data$INTERACT)) } 
  if(length(suspended_r) >0) { vec <- c(vec, as.character("Suspended")) 
  st <- c(st, as.character(data$SUSPENDED)) } 
  if(length(improved_r) >0) { vec <- c(vec, as.character("ImprovedAfterSuspension")) 
